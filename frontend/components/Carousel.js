...
// Carousel — a smooth, auto-advancing image gallery using a gentle CROSSFADE
// (the premium, Apple-style hero feel) rather than a hard slide. Slides are
// stacked and fade between each other with Apple's easing. Auto-advances,
// pauses on hover, and offers arrows + dot indicators.
//
// Props:
...
export default function Carousel({ slides, interval = 5000 }) {
  ...
  return (
    <div
      className="group relative aspect-[16/9] overflow-hidden rounded-3xl bg-gray-100 shadow-sm dark:bg-brand-slate"
    ...
    >
      {/* Stacked slides that crossfade between each other */}
      {slides.map((slide, i) => (
        <div
          key={i}
      ...
      <button
        ...
        onClick={() => go(index - 1)}
        className="absolute left-3 top-1/2 z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-gray-900 shadow transition hover:bg-white md:opacity-0 md:group-hover:opacity-100"
      >
      ...
      <button
        ...
        onClick={() => go(index + 1)}
        className="absolute right-3 top-1/2 z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/80 text-gray-900 shadow transition hover:bg-white md:opacity-0 md:group-hover:opacity-100"
      >
